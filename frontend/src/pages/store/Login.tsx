...
import { ShieldCheck, UserCheck, Lock, Eye, EyeOff, KeyRound, Mail, CheckCircle2, AlertCircle, RefreshCw, Loader2 } from 'lucide-react';
...
export default function Login({ setView }: LoginProps) {
  ...
  // Granular loading states — prevents multi-click and shows exact action
  ...
  const isAnyLoading = loadingLogin || loadingGoogle || loadingOtp || loadingSendOtp || loadingForgot || loadingReset || loadingVerify;
  ...
  useEffect(() => {
    ...
    if (requiresOTP === 'true' && paramEmail) {
      ...
      setInfoVal('🛡️ Multi-Factor OTP Verification required. A secure passcode has been sent to your registered email.');
    }
  }, []);
  ...
  const handleEmailVerification = async (tok: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setInfoVal('🙏 Account verified successfully! You can sign in now.');
      } else {
        setErrorVal(data.message || 'Verification failed. Signature is invalid or expired.');
      }
    } catch {
      setErrorVal('An error occurred while confirming the verification signature.');
    } finally {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (requestInFlight.current || loadingLogin) return;
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Access credentials invalid');

      if (data.requiresOTP) {
        setAuthTempUser(data);
        setMode('OTP_CHALLENGE');
        setInfoVal(data.message || '🛡️ Multi-Factor OTP Verification required. A secure passcode has been sent to your email.');
      } else {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (!popupWindow) throw new Error('Pop-up blocked. Please allow popups for this domain.');
  ...
  const handleOtpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (requestInFlight.current || loadingOtp) return;
    requestInFlight.current = true;
    setErrorVal('');
    setLoadingOtp(true);
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Security passcode verification failed.');
      googleLogin(data.accessToken, data.refreshToken || '', data.user);
      setView('admin');
    } catch (err: any) {
  ...
  const handleForgotSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      setInfoVal(data.message || 'If this account is registered, a reset link has been dispatched.');
    } catch {
  ...
  const handleResetSubmit = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setErrorVal('Mismatch: Passwords must be identical.');
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Failed to complete credentials reset.');
      setInfoVal('🎉 Password updated successfully! Please sign in with your new password.');
      setPassword('');
      setConfirmPassword('');
      setMode('LOGIN');
    } catch (err: any) {
  ...
  const handleSendOtp = async () => {
    if (!email || loadingSendOtp || cooldown > 0) return;
    setErrorVal('');
    setInfoVal('');
    ...
    try {
      ...
      setOtpSent(true);
      setCooldown(60);
      setInfoVal('🙏 Secure 6-digit passcode sent to your email. Check your spam folder too.');
    } catch (err: any) {
  ...
  const handleCustomerOtpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !otpCode || requestInFlight.current || loadingOtp) return;
    requestInFlight.current = true;
    setErrorVal('');
    setInfoVal('');
    setLoadingOtp(true);
      ...
      <div className="max-w-md w-full mx-4 bg-white border border-[#D4B896]/60 p-8 rounded-2xl shadow-sm flex flex-col items-center">
        ...
        {mode === 'LOGIN' && (
          <>
            ...
            <div className="flex w-full bg-stone-100 p-1 rounded-full mb-6 max-w-sm">
              ...
              <button type="button" onClick={() => { setIsOtpLogin(true); setErrorVal(''); setInfoVal(''); }}
                className={`flex-1 text-center py-2 rounded-full text-xs font-bold transition-all duration-150 cursor-pointer ${isOtpLogin ? 'bg-[#6B2D0E] text-white shadow-sm' : 'text-stone-500 hover:text-stone-700'}`}>
                Email OTP Access
              </button>
            </div>
            ...
            {!isOtpLogin ? (
              <form onSubmit={handleLoginSubmit} className="w-full flex flex-col gap-4 font-sans">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    <input type={showPassword ? 'text' : 'password'} required placeholder="••••••••" value={password}
                      ...
                      className="w-full bg-stone-50 border border-[#D4B896]/70 text-[#2C1810] text-xs font-medium rounded-lg p-2.5 pl-9 pr-9 focus:outline-none focus:ring-1 focus:ring-[#E8820C] disabled:opacity-60" />
                ...
                <button type="submit" disabled={loadingLogin || isAnyLoading}
                  className="w-full bg-[#6B2D0E] hover:bg-[#E8820C] disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-3 rounded-full shadow hover:shadow-md transition-all mt-3 select-none cursor-pointer text-xs flex items-center justify-center gap-2">
                  {loadingLogin ? (<><Spinner /> Signing In...</>) : 'Sign In Now'}
                </button>

                <div className="relative my-2">
                  <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[#D4B896]/30"></div></div>
                ...
                <button type="button" onClick={handleGoogleSignIn} disabled={isAnyLoading}
                  className="w-full flex items-center justify-center gap-2 bg-white border border-[#D4B896]/60 hover:bg-[#F5EFE6] disabled:opacity-60 disabled:cursor-not-allowed text-[#2C1810] font-bold py-2.5 px-4 rounded-full transition-all text-xs cursor-pointer">
                  {loadingGoogle ? (
              ...
              <form onSubmit={handleCustomerOtpSubmit} className="w-full flex flex-col gap-4 font-sans">
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <input type="email" required placeholder="e.g. seeker@vedic.com" value={email}
                        onChange={(e) => setEmail(e.target.value)} disabled={loadingSendOtp}
                        className="w-full bg-stone-50 border border-[#D4B896]/70 text-[#2C1810] text-xs font-medium rounded-lg p-2.5 pl-9 focus:outline-none focus:ring-1 focus:ring-[#E8820C] disabled:opacity-60" />
                      <UserCheck size={14} className="absolute left-3 top-3 text-stone-400" />
                    </div>
                    <button type="button" onClick={handleSendOtp}
                      disabled={loadingSendOtp || cooldown > 0 || !email}
                      className="bg-[#6B2D0E] hover:bg-[#E8820C] text-white font-bold text-xs px-4 py-2.5 rounded-lg disabled:opacity-50 select-none cursor-pointer duration-100 shrink-0 flex items-center gap-1.5">
                      {loadingSendOtp ? <><Spinner size={12} /> Sending...</> : cooldown > 0 ? `${cooldown}s` : otpSent ? 'Resend' : 'Send OTP'}
                    </button>
                  </div>
                </div>
                ...
                {otpSent && (
                  <div className="flex flex-col gap-1.5 animate-fadeIn">
                    <label className="text-[10px] font-bold uppercase text-stone-500">6-Digit OTP Code</label>
                    <div className="relative">
                      <input type="text" required maxLength={6} placeholder="Enter 6-digit Code" value={otpCode}
                ...
                <button type="submit" disabled={loadingOtp || !otpSent || otpCode.length < 6}
                  className="w-full bg-[#6B2D0E] hover:bg-[#E8820C] text-white font-bold py-3 rounded-full shadow hover:shadow-md transition-all mt-3 select-none cursor-pointer text-xs disabled:opacity-50 flex items-center justify-center gap-2">
                  {loadingOtp ? <><Spinner /> Verifying OTP...</> : 'Verify & Log In'}
                </button>
              </form>
            )}
          </>
        ...
        {mode === 'OTP_CHALLENGE' && (
          <form onSubmit={handleOtpSubmit} className="w-full flex flex-col gap-4 font-sans">
            <div className="text-center text-xs font-medium text-stone-500 mb-2 leading-relaxed">
              Enter the 6-digit OTP sent to your admin email to complete access clearance.
            </div>
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex gap-2 justify-center">
                {Array.from({ length: 6 }).map((_, i) => (
                  <input key={i} type="text" maxLength={1} inputMode="numeric"
                    ...
                    className="w-10 h-12 text-center text-xl font-bold border-2 border-amber-300 rounded-lg bg-stone-50 text-[#2C1810] focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-400 transition-all"
                  />
                ))}
              </div>
            </div>

            <button type="submit" disabled={loadingOtp || otpCode.length < 6}
              className="w-full bg-amber-600 hover:bg-amber-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold py-3 rounded-full shadow hover:shadow-md transition-all mt-2 select-none cursor-pointer text-xs flex items-center justify-center gap-2">
              {loadingOtp ? <><Spinner /> Authorizing...</> : 'Confirm Authentication'}
            </button>

            <button type="button" onClick={() => { setMode('LOGIN'); setOtpCode(''); setAuthTempUser(null); setErrorVal(''); }}
        ...
        {mode === 'FORGOT_PASSWORD' && (
          <form onSubmit={handleForgotSubmit} className="w-full flex flex-col gap-4 font-sans">
            <div className="text-xs text-stone-500 leading-normal mb-2">
              Enter your registered email. We'll dispatch a secure password recovery link.
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-bold uppercase text-stone-500">Contact Email Address</label>
              <div className="relative">
            ...
            <button type="submit" disabled={loadingForgot}
              className="w-full bg-[#6B2D0E] hover:bg-[#E8820C] disabled:opacity-60 text-white font-bold py-3 rounded-full shadow hover:shadow-md transition-all mt-2 select-none cursor-pointer text-xs flex items-center justify-center gap-2">
              {loadingForgot ? <><Spinner /> Dispatching Reset Link...</> : 'Dispatch Reset Directions'}
            </button>
            <button type="button" onClick={() => { setMode('LOGIN'); setErrorVal(''); setInfoVal(''); }}
        ...
        {mode === 'RESET_PASSWORD' && (
          <form onSubmit={handleResetSubmit} className="w-full flex flex-col gap-4 font-sans">
            <div className="flex flex-col gap-1.5">
              ...
              <input type="password" required placeholder="At least 8 characters with Capital, Digit & Symbol"
                value={password} onChange={(e) => setPassword(e.target.value)} disabled={loadingReset}
                className="w-full bg-stone-50 border border-[#D4B896]/70 text-[#2C1810] text-xs font-medium rounded-lg p-2.5 focus:outline-none focus:ring-1 focus:ring-[#E8820C] disabled:opacity-60" />
            </div>
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] font-bold uppercase text-stone-500">Confirm New Password</label>
              <input type="password" required placeholder="Confirm Password"
                value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={loadingReset}
                className="w-full bg-stone-50 border border-[#D4B896]/70 text-[#2C1810] text-xs font-medium rounded-lg p-2.5 focus:outline-none focus:ring-1 focus:ring-[#E8820C] disabled:opacity-60" />
            </div>
            <button type="submit" disabled={loadingReset || !password || password !== confirmPassword}
              className="w-full bg-emerald-700 hover:bg-emerald-800 disabled:opacity-60 text-white font-bold py-3 rounded-full shadow hover:shadow-md transition-all mt-2 select-none cursor-pointer text-xs flex items-center justify-center gap-2">
              {loadingReset ? <><Spinner /> Saving New Password...</> : 'Save Fresh Password'}
            </button>
          </form>
        )}

        {/* ── 5. VERIFY EMAIL PENDING ── */}
        {mode === 'VERIFY_PENDING' && (
          <div className="w-full flex flex-col items-center justify-center p-6 text-center gap-4">
            {loadingVerify && <RefreshCw className="text-[#E8820C] animate-spin h-10 w-10" />}
            <div className="text-xs text-stone-500 font-medium">
              Verifying security token. Please wait...
            </div>
            <button type="button" onClick={() => { setMode('LOGIN'); setErrorVal(''); setInfoVal(''); }}
        ...
        {mode === 'LOGIN' && (
          <p className="text-xs text-stone-400 mt-8 font-sans font-medium">
            ...
            <button onClick={() => setView('register')} className="text-[#6B2D0E] hover:text-[#E8820C] font-bold underline cursor-pointer">
              Register Account
            </button>
          </p>
        )}
