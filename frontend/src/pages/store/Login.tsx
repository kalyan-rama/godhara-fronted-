...
export default function Login({ setView }: LoginProps) {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if user has administrative roles. If so, they must go through the 2FA portal.
      const isAdminRole = ['SUPER_ADMIN', 'ADMIN', 'MODERATOR', 'VIEWER'].includes(data.user.role);
      
      if (isAdminRole) {
        setAuthTempUser(data); // save temporarily
        setMode('OTP_CHALLENGE');
        setInfoVal('🛡️ Multi-Factor OTP Verification required for administrator accounts. Demo Code is "108108".');
        setLoading(false);
      } else {
        ...
        {mode === 'LOGIN' && (
          <>
            ...
            <div className="w-full bg-[#F5EFE6]/60 rounded-xl border border-[#D4B896]/30 p-3.5 text-[10px] text-stone-500 font-sans leading-normal mb-6 flex flex-col gap-1">
              ...
              <p><strong>Admin Session (2FA OTP is 108108):</strong> kalyanvasantham906@gmail.com | <strong>Pass:</strong> admin123</p>
              <p><strong>Customer Session:</strong> seeker@vedic.com | <strong>Pass:</strong> admin123</p>
            </div>
