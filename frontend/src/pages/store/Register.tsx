...
import { ClipboardList, Check, X, ShieldCheck, RefreshCw, KeyRound } from 'lucide-react';
...
export default function Register({ setView }: RegisterProps) {
  ...
  // Step 1: Submit form → send OTP, do NOT create account
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Google account upgrade — handled immediately, no OTP needed
      if (!data.requiresOTP) {
      ...
      setStep('OTP_VERIFY');
      setCooldown(60);
      setSuccessVal('✅ Verify your OTP to complete registration. A 6-digit code has been sent to your email.');
    } catch (err: any) {
    ...
    try {
      ...
      const handleMessage = (event: MessageEvent) => {
        if (event.data?.type === 'OAUTH_AUTH_SUCCESS') {
          const { accessToken, refreshToken, user: authUser, requiresOTP, email: authEmail } = event.data;
          if (requiresOTP) {
            // Admin Google login — go to OTP challenge in login page
      ...
      const pollTimer = setInterval(() => {
        if (popupWindow.closed) {
          ...
          setLoading((prev) => { if (prev) { window.removeEventListener('message', handleMessage); return false; } return false; });
        }
      }, 1000);
    } catch (err: any) {
        ...
        {step === 'FORM' && (
          <form onSubmit={handleFormSubmit} className="w-full flex flex-col gap-3 font-sans">
            <div className="flex flex-col gap-1">
              ...
              <input type="text" required placeholder="e.g. Laxman Rao" value={name} onChange={(e) => setName(e.target.value)}
            ...
            <div className="flex flex-col gap-1">
              ...
              <input type="password" required placeholder="Min 8 characters, capital, digit & symbol" value={password} onChange={(e) => setPassword(e.target.value)}
                className="bg-stone-50 border border-[#D4B896]/70 text-[#2C1810] text-xs font-medium rounded-lg p-2 focus:outline-none focus:ring-1 focus:ring-[#E8820C]" />
              {password.length > 0 && (
                <div className="mt-1 bg-stone-50 border border-stone-100 rounded-lg p-2.5 flex flex-col gap-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 mt-1 text-[9px] font-medium text-stone-500">
                    ...
                    <span className="flex items-center gap-1">{hasNumber ? <Check size={10} className="text-emerald-600" /> : <X size={10} className="text-red-500" />} Digit number (0-9)</span>
                    <span className="flex items-center gap-1">{hasSymbol ? <Check size={10} className="text-emerald-600" /> : <X size={10} className="text-red-500" />} Symbol punctuation</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <input type="tel" required placeholder="e.g. +91 9999999999" value={phone} onChange={(e) => setPhone(e.target.value)}
                className="bg-stone-50 border border-[#D4B896]/70 text-[#2C1810] text-xs font-medium rounded-lg p-2 focus:outline-none focus:ring-1 focus:ring-[#E8820C]" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1 col-span-2">
                ...
                <input type="text" placeholder="Temple compound" value={street} onChange={(e) => setStreet(e.target.value)}
                  className="bg-stone-50 border border-[#D4B896]/70 text-[#2C1810] text-xs font-medium rounded-lg p-2 focus:outline-none focus:ring-1 focus:ring-[#E8820C]" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold uppercase text-stone-500">City / Village</label>
                <input type="text" placeholder="Hyderabad" value={city} onChange={(e) => setCity(e.target.value)}
                  className="bg-stone-50 border border-[#D4B896]/70 text-[#2C1810] text-xs font-medium rounded-lg p-2 focus:outline-none focus:ring-1 focus:ring-[#E8820C]" />
              </div>
              <div className="flex flex-col gap-1">
                <label className="text-[10px] font-bold uppercase text-stone-500">PIN Code</label>
                <input type="text" placeholder="500001" value={pincode} onChange={(e) => setPincode(e.target.value)}
                  className="bg-stone-50 border border-[#D4B896]/70 text-[#2C1810] text-xs font-medium rounded-lg p-2 focus:outline-none focus:ring-1 focus:ring-[#E8820C]" />
              </div>
            </div>

            <button type="submit" disabled={loading || strengthScore < 100 || !passwordsMatch}
              className={`w-full text-white font-bold py-3 rounded-full shadow hover:shadow-md transition-all mt-4 select-none cursor-pointer text-xs ${
                loading || strengthScore < 100 || !passwordsMatch ? 'bg-stone-300 cursor-not-allowed opacity-75' : 'bg-[#6B2D0E] hover:bg-[#E8820C]'
              }`}>
        ...
        {step === 'OTP_VERIFY' && (
          <div className="w-full flex flex-col items-center gap-4">
            ...
            <p className="text-center text-xs text-stone-600 leading-relaxed">
              A <strong>6-digit OTP</strong> has been sent to<br />
              <strong className="text-[#6B2D0E]">{pendingEmail}</strong><br />
              Enter it below to complete your registration. Valid for <strong>10 minutes</strong>.
            </p>
            ...
            <form onSubmit={handleOtpVerify} className="w-full flex flex-col gap-3">
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  className="bg-stone-50 border border-[#D4B896]/70 text-[#2C1810] text-center text-2xl font-bold tracking-[0.5em] rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-[#6B2D0E]"
                />
              </div>

              <button type="submit" disabled={loading || otpCode.length !== 6}
                className={`w-full text-white font-bold py-3 rounded-full shadow transition-all text-xs ${
                  loading || otpCode.length !== 6 ? 'bg-stone-300 cursor-not-allowed opacity-75' : 'bg-[#6B2D0E] hover:bg-[#E8820C]'
                }`}>
            ...
            <div className="flex items-center gap-2 text-xs text-stone-500">
              ...
              {cooldown > 0 ? (
                ...
                <button onClick={handleResendOtp} disabled={loading}
                  className="text-[#6B2D0E] font-bold hover:text-[#E8820C] underline cursor-pointer disabled:opacity-50">
                  Resend OTP
                </button>
              )}
            </div>
